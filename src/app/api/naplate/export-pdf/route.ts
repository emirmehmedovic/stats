...
export async function GET(request: NextRequest) {
  try {
    ...
    const periodStart = date ? parseDateOnly(date) : null;
    if (!periodStart) {
      return NextResponse.json({ error: 'Nedostaje datum' }, { status: 400 });
    }

    const reportRecord = await prisma.billingReport.findUnique({
      where: {
        type_periodStart: {
          type: 'DAILY',
          periodStart,
    ...
    if (!reportRecord?.data) {
      return NextResponse.json({ error: 'Izvještaj ne postoji' }, { status: 404 });
    }

    const report = normalizeDailyReport(reportRecord.data as DailyReport);
    const html = generatePDFHTML(report);
    const fileName = `Dnevni-izvjestaj-${date}.html`;
    ...
    console.error('Export naplate PDF error:', error);
    return NextResponse.json({ error: 'Greška pri eksportu PDF-a' }, { status: 500 });
  }
}

function generatePDFHTML(report: DailyReport): string {
...
<head>
  ...
  <title>Dnevni naplatni izvještaj</title>
  <style>
    @media print {
...
<body>
  ...
  <div class="header">
    ...
    <div>
      <h1>Dnevni naplatni izvještaj</h1>
    </div>
  </div>
